...
public class App {
	...
	public static void main(String[] args) throws Exception {
		...
		OAuthRestTemplate restTmpl = new OAuthRestTemplate(new TokenRequester() {
			...
			public TokenHolder requestToken() throws Exception {
				...
				Request request = new Request.Builder().url("http://localhost:8080/oauth/token")
						...
						.addHeader("authorization", "Basic Y2xpZW50YXBwOjEyMzQ1Ng==").post(requestBody).build();
		...
		// RequestBody requestBody = new FormEncodingBuilder().add("username",
		// "user").add("password", "pass")
		// .add("scope", "read").add("client_secret", "123456").add("client_id",
		// "clientapp")
		// .add("grant_type", "password").build();
		//
		// Request request = new
		// Request.Builder().url("http://localhost:8080/oauth/token")
		...
		//
		// ObjectMapper mapper = new ObjectMapper();
		// TokenHolder tokenHolder = mapper.readValue(jsonToken,
		// TokenHolder.class);
		// return tokenHolder;
		// });
		...
		String userData = restTmpl.makeCall(new ServiceCaller() {
			...
			public <String> String call(TokenHolder token) throws Exception {
				Request request = new Request.Builder().url("http://localhost:8080/user")
						.addHeader("Authorization", "Bearer " + token.getAccessToken()).get().build();
				Response response = http.newCall(request).execute();
